    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            with tf.device(device):
                ...
                cpmodel, model_in = get_model(args.model_name, batch_size, include_top=True, double_prec=False)
                remain = backend.function([cpmodel.layers[7].input], [cpmodel.layers[-1].output])
            ...
            for i in range(num_batches):
                ...
                #print(model.layers[8], model.layers[9])
                with tf.device(device):
                    
                    t_model = Sequential()
                    for i in range(9):
                        t_model.add(model.layers[i])

                    l = len(cpmodel.layers)
                    for i in range(l):
                        if i >= 7:
                            t_model.add(cpmodel.layers[i])

                preds = sess.run(t_model.output, feed_dict={t_model.inputs[0]: images,
                ...
                #with tf.device(device):
                #    remain([model.layers[8].output.eval()])
