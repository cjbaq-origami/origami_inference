...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            num_linear_layer=40
            partitionAtLayer = 9
            model_json_part1, weights_part1, model_json_part2, weights_part2 = model_to_json(sess, model, verif_preproc=True, dtype=dtype, bits_w=model_info['bits_w'], bits_x=model_info['bits_x'], partitionAtLayer=partitionAtLayer)
            sgxutils.load_model(model_json_part1, weights_part1, dtype=dtype, verify=True, verify_preproc=True)
            ...
            for i in range(num_batches):
                ...
                if blinded and not simulate:
                    prepare_blinding_factors(sess, model, sgxutils, in_ph, zs, out_ph, queue_ops, batch_size, bp=num_linear_layer #num_batches=1,inputs=images, temps=temps, out_funcs=out_funcs
                    )
                ...
                intermediates = tf.zeros([batch_size,56,56,256])
                preds = sess.run(model.output, feed_dict={model.inputs[0]: images,
                                backend.learning_phase(): 0},
                                options=run_options, run_metadata=run_metadata)
                #preds = first_half_model([images])
