    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            with tf.device(device):
                ...
                #sys.exit(0)
                get_final_layer_model_output = backend.function([model.input], [model.layers[-1].output])
            ...
            print("number of intermediate features: {}".format(num_inter_feats))
                ...
                if args.mode in ['tf-gpu', 'tf-cpu']:
                    ...
                    if args.verify:
                        ...
                        for i in range(args.batch_size):
                            ...
                            print("intermediate features shape: {}".format(intermediate_feats.shape))
                            ...
                            # Predict partition 2 outside enclave, in GPU
                                pred = get_final_layer_model_output([intermediate_feats])[0]
                            print("pred shape: {}".format(pred.shape))
                            print("GPU compute time: {}".format(time.time() - start_inter_time))
